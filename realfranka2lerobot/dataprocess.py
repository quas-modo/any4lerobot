...
data_sources = [
"/oss/shared/tr-eval/lianghaotian/franka_real_data/stack_cup",
]


npy_path = "/oss/shared/tr-eval/lianghaotian/franka_real_data/stack_cup_npy"
...
for file_path in data_sources:
    task_instruction = file_path.split("/")[-1].replace("_"," ")
    print(task_instruction)
    h5_files = glob.glob(f'{file_path}/**/*.h5', recursive=True) + glob.glob(f'{file_path}/**/*.hdf5', recursive=True)
    episode = []
    print(len(h5_files))

    if not os.path.exists(npy_path):
        os.makedirs(npy_path)
        print(f"创建目录: {npy_path}")
    else:
        print(f"目录已存在: {npy_path}")
    ...
    for _, i in tqdm(enumerate(h5_files)):
        ...
        for task_index, sub_task in enumerate(data_subtasks):
            ...
            non_zero_camera_images_3 = sub_task['camera_3_image'][non_zero_delta_indices]
            non_zero_camera_images_3_a = sub_task['camera_3_image_a'][non_zero_delta_indices]
            ...
            assert non_zero_camera_images_3.shape[0] == non_zero_state_processed_data.shape[0], \
                "Camera images and actions are misaligned in number of frames!"
            assert non_zero_camera_images_3_a.shape[0] == non_zero_state_processed_data.shape[0], \
                "Camera images and actions are misaligned in number of frames!"
            print(non_zero_camera_images.shape)
            print(non_zero_action_delta_data.shape,non_zero_state_processed_data.shape)


            # if non_zero_camera_images.shape[0] > 0:
            #     first_image_array = non_zero_camera_images[0]
            #     first_image = Image.fromarray(first_image_array, 'RGB')
            #     first_image.save("first_camera_1_image.png")
            #     print("已保存 non_zero_camera_images 的第一张图片为 first_camera_1_image.png")
            # else:
            #     print("non_zero_camera_images 中没有图片可供保存。")
            # # 验证并保存 non_zero_camera_images_3 的第一张图片
            # if non_zero_camera_images_3.shape[0] > 0:
            #     first_image_3_array = non_zero_camera_images_3[0]
            #     # 将 NumPy 数组转换为 Pillow Image 对象
            #     # 如果你的数组是 uint8 类型，mode='RGB' 是合适的
            #     first_image_3 = Image.fromarray(first_image_3_array, 'RGB')
                
            #     # 保存图像到文件
            #     first_image_3.save("first_camera_3_image.png")
            #     print("已保存 non_zero_camera_images_3 的第一张图片为 first_camera_3_image.png")
            # else:
            #     print("non_zero_camera_images_3 中没有图片可供保存。")

            # # 验证并保存 non_zero_camera_images_3_a 的第一张图片
            # if non_zero_camera_images_3_a.shape[0] > 0:
            #     first_image_3_a_array = non_zero_camera_images_3_a[0]
            #     first_image_3_a = Image.fromarray(first_image_3_a_array, 'RGB')
                
            #     # 保存图像到文件
            #     first_image_3_a.save("first_camera_3_a_image.png")
            #     print("已保存 non_zero_camera_images_3_a 的第一张图片为 first_camera_3_a_image.png")
            # else:
            #     print("non_zero_camera_images_3_a 中没有图片可供保存。")
            ...
            results = {
                ...
                'image': non_zero_camera_images,        # Shape: (N, 256, 256, 3)
                'image_3': non_zero_camera_images_3,        # Shape: (N, 256, 256, 3)
                'image_3_a': non_zero_camera_images_3_a,        # Shape: (N, 256, 256, 3)
            }
